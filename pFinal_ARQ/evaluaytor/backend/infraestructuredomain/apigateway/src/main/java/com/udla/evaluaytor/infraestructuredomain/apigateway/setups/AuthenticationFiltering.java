          ...
          .map(response -> {
            if (response != null) {
              ...
              // check for Partners rol
               if(response.get("user_evaluaytor") == null || StringUtils.isEmpty(response.get("user_evaluaytor").asText())){
                throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Role user_evaluaytor missing");
              }
            } else {
